...
int main(void) {
    if (logger_init() != 0) {
        fprintf(stderr, "Błąd: logger nie został zainicjalizowany\n");
        exit(EXIT_FAILURE);
    }
    printf("Logger zainicjalizowany. PID: %d\n", getpid());
    printf("SIGRTMIN   - generuje dump\n");
    printf("SIGRTMIN+1 - przełącza logowanie\n");
    printf("SIGRTMIN+2 - zmienia poziom logowania\n");
    printf("Ctrl+C konczy dzialanie programu\n");

    logger_run();

    printf("Logger: zakończenie pracy...\n");
    logger_cleanup();
